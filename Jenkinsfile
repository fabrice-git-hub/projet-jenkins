pipeline {
    ...
    stages {
        ...
        stage ('Deploy Review'){
            environment {
                SERVER_IP = '3.95.196.81'
                SERVER_USERNAME = 'ubuntu'
            }
            steps {
                script{
                    timeout(time: 30, unit: "MINUTES"){
                        input message: "Voulez-vous réaliser un deploiement sur la Review ?", ok: 'Yes'
                    }
                    sshagent(['key-pair']){
                        sh '''
                            echo $DOCKER_PASSWORD | docker login -u $DOCKER_USERNAME --password-stdin
                            ...
                            sleep 30
                            ssh -o StrictHostKeyChecking=no -l  $SERVER_USERNAME $SERVER_IP "docker run --rm -dp $HOST_PORT:$CONTAINER_PORT --name $DOCKER_USERNAME/$IMAGE_NAME:$IMAGE_TAG"
                            sleep 5
        ...
        stage ('Deploy Staging'){
            environment {
                SERVER_IP = '3.93.163.223'
                SERVER_USERNAME = 'ubuntu'
            }
            steps {
                script{
                        timeout(time: 30, unit: "MINUTES"){
                            input message: "Voulez-vous réaliser un deploiement sur le Staging ?", ok: 'Yes'
                    }
                    sshagent(['key-pair']){
                        sh '''
                            echo $DOCKER_PASSWORD | docker login -u $DOCKER_USERNAME --password-stdin
        ...
        stage ('Deploy Prod'){
            ...
            environment {
                SERVER_IP = '54.227.109.70'
                SERVER_USERNAME = 'ubuntu'
            }
            steps {
                script{
                        timeout(time: 30, unit: "MINUTES"){
                            input message: "Voulez-vous réaliser un deploiement sur la PROD ?", ok: 'Yes'
                    }
                    sshagent(['key-pair']){
                        sh '''
                            echo $DOCKER_PASSWORD | docker login -u $DOCKER_USERNAME --password-stdin
